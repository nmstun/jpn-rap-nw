...
- センターと直接共演した曲数(次数)が多い上位3組(共演1曲のみの相手は除く)については、さらにその客演相手(2hop先)まで展開します。
...
## 長時間処理の扱い（キャンセル方法）
- 人気アーティストのネットワーク構築は数十秒かかることがあります。2hop展開(上位3組)が走ると、さらに数十秒〜1分程度延びることがあります。フロントは `AbortController` による検索キャンセルを実装しています。
