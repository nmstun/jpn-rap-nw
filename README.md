...
## 長時間処理の扱い(ジョブ化・ポーリング)
- 人気アーティストのネットワーク構築は数十秒から数分かかることがあります(hop数を2・3に増やすと、各hopで最大3組ずつ追加展開されるため3hopでは最大1+3+9=13アーティスト分)。レンダー/ブラウザのHTTPタイムアウトに当たるのを避けるため、同期リクエストではなくジョブ化しています。
- `POST /api/network` は即座に `jobId` を返し(202)、実際の構築はバックグラウンドで進む。フロントは `GET /api/network/:jobId` を1秒間隔でポーリングし、`status`(`pending`/`running`/`done`/`error`/`cancelled`)と `progress`(何hop目・何組展開済みか)を取得してローディング表示に反映します。`done` のレスポンスのみ `result` を含みます。
- キャンセルは `POST /api/network/:jobId/cancel` で行います。フロントの「キャンセル」ボタンはこれを呼んでサーバー側のジョブに `cancelled` フラグを立て、ジョブ側は次のチェックポイント(曲詳細取得の前後・hop展開の前後)で早期終了するため、即座には止まらないことがあります。
- ジョブはプロセス内メモリ(`Map`)で保持しています(Renderは基本1インスタンス構成のため)。完了/失敗/キャンセル後も一定時間(10分)はポーリング可能で、その後自動破棄されます。複数インスタンスに展開する場合は外部ストア(Redis等)への置き換えが必要です。
